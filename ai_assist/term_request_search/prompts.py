...
Required structural search process:
    ...
    3. After the user selects an ontology, call find_category_terms with that ontologyId. It searches paginated matches in the selected ontology for the requested category and all of its synonyms. Choose the best matching class or property as the category anchor. The category may be a descendant of a root, so do not assume it is a root term.
    4. Starting from the category anchor, use the requested label and definition to choose each next layer with search_in_children. The function searches all direct children locally and returns the closest match. If that match remains a broader concept, search its children next.
...
Traversal rules:
    - Do not use batch_search, search_under_term, or get_individuals. Use find_category_terms only to locate the category anchor; after that, use ontology structure.
    - Explore only the ontology selected by the user. Use that same ontologyId for every find_category_terms, get_roots, search_in_children, and get_term_detail call, including for terms imported from another ontology. Never switch traversal to the imported term's source ontology.
    - The traversal state in your context lists the selected ontology, category anchors, visited root pages, and visited nodes. Expand only category anchors or terms returned by get_roots or search_in_children. Never search the same node twice; backtrack to a different returned term instead.
    ...
    - Treat category compatibility as a hard constraint throughout traversal. Never enter or remain in a subtree that conflicts with the requested category merely because its labels resemble the requested term.
    - If find_category_terms returns no suitable category anchor, use get_roots and traverse with only the category or one of its synonyms as the search_in_children query until an exact category anchor is reached. Do not use the requested term label, return fallback nodes as candidates, or fall back to a different category.
...
- functions you can call are: 
    ...
    - find_category_terms(ontologyId): search paginated results in the selected ontology for exact label or synonym matches to the requested category. It returns an object keyed by query whose values are matching class or property dictionaries. Call it once before parent traversal and choose a category-compatible term as the anchor.
