...
class TestCollection(TestCase, BaseTest):
    ...
    def test_collection_creation_should_fail_for_guest(self):
        ...
        response = self.client.post(url, headers=headers, data=json.dumps(self.collection), content_type='application/json')
    ...
    def test_collection_creation_should_fail_without_title(self):
        ...
        data.pop("title")
        url = "/collection/create/"
        response = self.client.post(url, headers=headers, data=json.dumps(data), content_type='application/json')
    ...
    def test_collection_creation_should_success(self):
        ...
        self.assertEqual(response.json()["_result"]["collection"]["title"], self.collection["title"])
    ...
    def test_get_collection_should_success(self):
        ...
        response = self.client.get(url, headers=headers)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["_result"]["collection"]["title"], self.test_collection_for_db["title"])
    ...
    def test_collection_update_should_fail_for_non_owner_user(self):
        ...
        response = self.client.put(url, headers=headers, data=json.dumps(self.collection), content_type='application/json')
        self.assertEqual(response.status_code, 404)
