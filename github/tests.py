...
class TestTermRequest(BaseTest):

    @classmethod
    def setUpTestData(cls) -> None:
        super().setUpTestData()
        cls.term_request_data = {
            "title": "test issue",
        ...
        cls.term_request_url = "/github/submit_issue/"
    ...
    def test_term_request_should_fail_for_orcid_user(self):
        ...
        self.assertIn(
            "Only github users can use this feature",
            response.json()["_result"]["error"],
        )
    ...
    def test_term_request_list_should_success(self):
        ...
        response = self.client.get(
            "/github/get_submited_issues/", headers=self.github_request_headers
        )
        self.assertEqual(response.status_code, 200)
        self.assertGreater(len(response.json()["_result"]["submited_issues"]), 0)
        self.assertEqual(
            response.json()["_result"]["submited_issues"][0]["ontology_id"], "vibso"
        )
