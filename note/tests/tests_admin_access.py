...
class TestAdminAccess(TestCase, BaseTest):
    ...
    def setUpTestData(self) -> None:
        ...
        self.internal_vis_note_comment = TestHelper.createCommentForNote(self.orcidUser, self.internal_vis_note)
        self.me_vis_note_comment = TestHelper.createCommentForNote(self.orcidUser, self.me_vis_note)
    ...
    def test_note_update_admin_should_work_for_admin(self):
        ...
        response = self.client.put(note_update_url, headers=headers, data=json.dumps(self.edit_internal_note_data), content_type="application/json")        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['_result']['note_updated']['title'], "Edited internal")
    


    def test_note_comment_admin_edit_should_success(self):
        headers = copy.copy(self.github_request_headers)
        post_data = {"content": "edited comment by admin", "ontology_id": self.test_ontology_id}
        post_data['comment_id'] = self.internal_vis_note_comment.id
        comment_update_url = '/note/update_comment/'
        response = self.client.put(comment_update_url, headers=headers, data=json.dumps(post_data), content_type="application/json")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['_result']['comment_updated']['content'], "edited comment by admin")
    


    def test_delete_admin_should_success_for_note(self):
        headers = copy.copy(self.github_request_headers)
        post_data = {"objectId": self.edit_internal_note_data['noteId'], "objectType": "note", "ontology_id": self.test_ontology_id}
        note_comment_delete_url = '/note/delete/'
        response = self.client.delete(note_comment_delete_url, headers=headers, data=json.dumps(post_data), content_type="application/json")
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_admin_should_success_for_comment(self):
        headers = copy.copy(self.github_request_headers)
        post_data = {"objectId": self.internal_vis_note_comment.id, "objectType": "comment", "ontology_id": self.test_ontology_id}
        note_comment_delete_url = '/note/delete/'
        response = self.client.delete(note_comment_delete_url, headers=headers, data=json.dumps(post_data), content_type="application/json")        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['_result']['deleted'], True)
    ...
    def test_delete_admin_should_fail_for_me_note(self): 
        headers = copy.copy(self.github_request_headers)
        post_data = {"objectId": self.edit_me_note_data['noteId'], "objectType": "note", "ontology_id": self.test_ontology_id}
        note_comment_delete_url = '/note/delete/'
        response = self.client.delete(note_comment_delete_url, headers=headers, data=json.dumps(post_data), content_type="application/json")
    ...
    def test_delete_admin_should_fail_for_me_comment(self):
        headers = copy.copy(self.github_request_headers)
        post_data = {"objectId": self.me_vis_note_comment.id, "objectType": "comment", "ontology_id": self.test_ontology_id}
        note_comment_delete_url = '/note/delete/'
        response = self.client.delete(note_comment_delete_url, headers=headers, data=json.dumps(post_data), content_type="application/json")        
        self.assertEqual(response.status_code, 401)
