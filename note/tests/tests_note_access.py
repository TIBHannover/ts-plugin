...
class TestNoteAccess(TestCase, BaseTest):
    ...
    def setUpTestData(self) -> None:
        ...
        self.note_for_another_ontology = TestHelper.createNote(
            user=self.gitHubUser, ontology_id="chmo"
        )
        self.public_note_comment = TestHelper.createCommentForNote(
            self.gitHubUser, self.public_note
        )
    ...
    def test_note_list_should_only_contains_public_note_for_guest_user(self):
        ...
        response = self.client.get(
            note_list_url, headers=self.guest_request_headers, data=params
        )
    ...
    def test_note_list_should_not_contains_child_notes_for_parent_ontology_when_onlyOriginalNotes_is_given(
        self,
    ):
    ...
    def test_note_list_stats(self):
        ...
        self.assertEqual(
            response.json()["_result"]["stats"]["total_number_of_records"], 4
        )
    ...
    def test_guest_user_cannot_access_private_note(self):
        ...
        response = self.client.get(
            get_a_note_url, headers=self.guest_request_headers, data=params
        )
        self.assertEqual(response.status_code, 404)
        self.assertIn("Note does not exist", response.content.decode())
