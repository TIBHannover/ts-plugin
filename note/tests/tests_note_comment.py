...
class TestNoteComment(TestCase, BaseTest):
    ...
    def setUpTestData(self) -> None:
        ...
        self.comment_to_edit = TestHelper.createCommentForNote(user=self.gitHubUser, note=self.public_note)
    ...
    def test_note_comment_should_fail_with_wrong_noteId(self):
        ...
        response = self.client.post(self.comment_create_url, headers=headers, data=json.dumps(post_data), content_type='application/json')
    ...
    def test_note_comment_should_success(self):    
        ...
        self.assertEqual(response.json()['_result']['comment_created']['content'], "Test Comment")
    ...
    def test_note_comment_edit_should_fail_with_wrong_commentId(self):
        ...
        response = self.client.put(self.comment_update_url, headers=headers, data=json.dumps(post_data), content_type='application/json')
        self.assertEqual(response.status_code, 404)
    ...
    def test_note_comment_edit_should_success(self):   
        ...
        post_data['comment_id'] = self.comment_to_edit.id
        response = self.client.put(self.comment_update_url, headers=headers, data=json.dumps(post_data), content_type='application/json')
        assert response.status_code == 200 and response.json()['_result']['comment_updated']['content'] == "edited"
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['_result']['comment_updated']['content'], "edited")
