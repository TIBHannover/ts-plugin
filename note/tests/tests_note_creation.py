...
class TestNoteCreation(TestCase, BaseTest):
    ...
    def test_note_creation_should_fail_without_client_ts_id(self):
        headers = copy.copy(self.github_request_headers)
        headers["X-TS-Frontend-Id"] = "other"
        response = self.client.post(self.note_creation_url, headers=headers, data=json.dumps(self.note), content_type='application/json')
        self.assertEqual(response.status_code, 401)
        self.assertIn("Client application is not allowed to use this service.", response.content.decode())


    def test_note_creation_should_fail_without_username(self):
        headers = copy.copy(self.github_request_headers)
        headers["X-TS-User-Name"] = "not_A_username"
        response = self.client.post( self.note_creation_url, headers=headers, data=json.dumps(self.note), content_type='application/json')
        self.assertEqual(response.status_code, 401)
    ...
    def test_note_creation_should_fail_without_title(self):
        ...
        del post_data["title"]
        response = self.client.post(self.note_creation_url, headers=headers, data=json.dumps(post_data), content_type='application/json')
    ...
    def test_note_creation_should_success(self):
        ...
        self.assertEqual("Test Note", response.json()["_result"]["note_created"]["title"]
)
    ...
    def test_note_creation_should_success_with_parent_ontology(self):
        ...
        response = self.client.post(self.note_creation_url, headers=headers, data=json.dumps(post_data), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["_result"]["note_created"]["parent_ontology"], self.test_parent_ontology_id)
