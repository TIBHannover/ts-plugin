...
class TestNoteUpdate(TestCase, BaseTest):
    ...
    def test_note_update_should_fail_without_noteId(self):
        ...
        response = self.client.put(self.note_update_url, headers=headers, data=json.dumps(post_data), content_type='application/json')
    ...
    def test_note_update_should_fail_for_guest_user(self):
        ...
        response = self.client.put(self.note_update_url, headers=headers, data=json.dumps(self.edit_note_data), content_type='application/json')
    ...
    def test_note_update_should_success_for_removing_parent_ontology(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["_result"]["note_updated"]["title"], "Edited")
        self.assertEqual(response.json()["_result"]["note_updated"]["parent_ontology"], None)
    ...
    def test_note_update_should_success_for_adding_parent_ontology(self):
        ...
        response = self.client.put(self.note_update_url, headers=headers, data=json.dumps(post_data), content_type='application/json')
        self.assertEqual(response.json()["_result"]["note_updated"]["parent_ontology"], "chmo")
