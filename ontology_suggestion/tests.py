...
class TestOntologySuggestion(TestCase, BaseTest):
    ...
    def test_onto_suggest_should_success(self):
        ...
        response = self.client.post(url, headers=headers, data=json.dumps(self.ontology_suggestion), content_type="application/json")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["_result"]["response"], "ontology is suggested successfully")
