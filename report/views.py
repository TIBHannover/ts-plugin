...
def create_report(request):
    ...
    user = UserModel.objects.filter(
        username=creator_username, client_ts=client_id
    ).first()
    ...
    try:
        ...
        body = "This {} is reported by the user {}: \n\n".format(
            object_type, creator_username
        )
        body += "Report Message: \n"
        body += "{} \n\n URL: \n".format(content)
        reported_content_base_url = get_frontend_base_url()
        if object_type == "note":
            reported_content_base_url += "/ontologies/{}/notes?noteId={}".format(
                ontologyId, object_id
            )

        elif object_type == "comment":
            comment = NoteCommentModel.objects.get(id=object_id)
            reported_content_base_url += (
                "/ontologies/{}/notes?noteId={}&comment={}".format(
                    ontologyId, comment.note_id, object_id
                )
            )
...
def resolve_report(request):
    ...
    role_model = RoleModel.objects.filter(
    ...
    if action == "none":
        reports = ReportModel.objects.filter(
            reported_object_type=object_type, reported_object_id=object_id
        )
        reports.delete()
    ...
    if object_type == "note":
        ...
        if note:
            note.delete()
            reports = ReportModel.objects.filter(
                reported_object_type="note", reported_object_id=object_id
            )
            for rep in reports:
                rep.resolve()
            return create_json_response({"resolved": True})
    ...
    if object_type == "comment":
        ...
        if comment:
            comment.delete()
            reports = ReportModel.objects.filter(
                reported_object_type="comment", reported_object_id=object_id
            )
            for rep in reports:
                rep.resolve()
            return create_json_response({"resolved": True})
...
def report_list(request):
    ...
    for report in reports:
        rep_dict = {}
        rep_dict["reporter_username"] = UserModel.get_user_name_by_id(
        ...
        if report.reported_object_type == "note":
            ...
            rep_dict["reported_content_url"] = (
                reported_content_base_url
                + "/ontologies/{}/notes?noteId={}".format(
                    note.ontology_id, report.reported_object_id
                )
            )
            ...
            rep_dict["reported_content_url"] = (
                reported_content_base_url
                + "/ontologies/{}/notes?noteId={}&comment={}".format(
                    note.ontology_id, comment.note_id, report.reported_object_id
                )
            )
