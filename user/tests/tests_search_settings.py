...
class TestSearchSetting(TestCase, BaseTest):
    ...
    def test_setting_creation_should_fail_for_guest(self):
        ...
        response = self.client.post(self.url, headers=headers, data=json.dumps(self.settings_dict), content_type='application/json')
    ...
    def test_setting_creation_should_fail_without_title(self):
        ...
        data.pop("title")
        response = self.client.post(self.url, headers=headers, data=json.dumps(data), content_type='application/json')
    ...
    def test_setting_creation_should_success(self):
        ...
        self.assertEqual(response.json()["_result"]["saved"]["title"], self.settings_dict["title"])
    ...
    def test_get_setting_should_success(self):
        ...
        response = self.client.get(url, headers=headers)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["_result"]["setting"]["title"], self.test_setting_for_db["title"])



    def test_get_setting_list_should_success(self):
        headers = copy.copy(self.github_request_headers)
        response = self.client.get(self.url, headers=headers, content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_setting_update_should_fail_for_non_owner_user(self):
        ...
        headers = copy.copy(self.orcid_request_headers)
        url = self.url + str(self.test_setting_for_db["id"]) + '/'
        response = self.client.put(url, headers=headers, data=json.dumps(self.settings_dict), content_type='application/json')
        self.assertEqual(response.status_code, 404)
